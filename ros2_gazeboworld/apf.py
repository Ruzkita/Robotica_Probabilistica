...
def algorithm(current_position, goal, lidar_data, angle_min, angle_increment, current_orientation):
    ...
    threshold = 2.5     # Distância mínima para considerar um obstáculo
    ...
    if -goal_threshold < current_position.x - goal[0] < goal_threshold and -goal_threshold < current_position.y - goal[1] < goal_threshold:
        linear_velocity = 0.0
    ...
    attractive_field = k_atrativo * np.array([dx, dy])
    ...
    for i in range(num_angles):
        ...
        if 0 < distance < threshold:  # Obstáculo detectado dentro do alcance
            ...
            angle_global = angle_rad + current_orientation
            repulse_x = -repulse_factor * math.cos(angle_global)
            repulse_y = -repulse_factor * math.sin(angle_global)
